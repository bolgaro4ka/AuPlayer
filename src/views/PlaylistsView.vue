...
<template>
    <div>
        <div class="header">
            <h1>Плейлисты</h1>
            <p>Здесь можно управлять плеёлистами</p>
        </div>
        <!-- Тут список плэйлистов с возможностью создания -->
        <PlaylistList :playlists="playlists" />
        <button class="create-button" @click="isShowPlaylistCreator = !isShowPlaylistCreator">Создать плейлист</button>
        <Teleport to="body">
            <Modal v-if="isShowPlaylistCreator" @close="isShowPlaylistCreator = false">
                <div class="playlist-creator">
                    <div class="form-group">
                        <input v-model="newPlaylistName" placeholder="Название плейлиста" class="playlist-name-input" />
                    </div>
                    ...
                    <div class="search-tracks">
                        <input v-model="searchQuery" placeholder="Поиск треков..." class="search-input" />

                        <div class="tracks-list">
                            <div v-for="track in filteredTracks" :key="track.path" class="track-item">
                                <label class="track-select">
                                    <input type="checkbox" v-model="selectedPaths" :value="track.path" />
